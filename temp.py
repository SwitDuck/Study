...
#ООП и классы
'''
Наследование: Представьте, что у нас есть общая категория "Роботы", которая определяет общие свойства и способности для всех 
роботов (например, они могут двигаться). Роботы по приготовлению пиццы – это особый вид роботов, который наследует все общие
свойства роботов (движение) и может добавлять свои уникальные возможности (например, приготовление пиццы). 
В наследовании основная идея в том, что нам не нужно переписывать общий код для каждого типа роботов – они берут его от 
родительского класса "Робот".

Композиция: Робот по приготовлению пиццы состоит из разных частей: манипуляторов для работы с тестом, двигателя для перемещения и т.д.
Это пример композиции – когда объекты создаются как совокупность других объектов, и каждый объект отвечает за свою часть работы. 
В композиции мы комбинируем разные классы, создавая более сложные структуры.

Множество экземпляров: Когда создается объект класса, он как бы “штампуется” из общего шаблона (класса). 
Каждый такой объект имеет свои данные, но использует общие методы класса. Например, если мы создадим два робота из класса
"Робот по приготовлению пиццы", они будут похожи, но каждый из них будет иметь свои уникальные данные (например, 
один готовит пиццу с сыром, а другой с грибами).

Настройка через наследование: Если у нас есть класс "Робот", и мы хотим добавить новые функции, мы можем создать подкласс 
"Робот по приготовлению пиццы" и расширить или изменить его поведение, добавив новые методы. Это позволяет легко настраивать
классы под конкретные задачи, не изменяя сам основной класс.

Перегрузка операций: Классы могут изменять поведение встроенных операций Python. Например, вы можете создать класс, 
который определяет, как его объекты будут вести себя при сложении (+), индексировании ([]) или других операциях. 
Это похоже на то, как работают встроенные типы данных, но вы можете настроить это под свои объекты.

Метод__ init__ известен как конструктор из-за момента своего запуска. Он явля­
ется самым часто используемым представителем крупной группы методов, называемых 
методами перегрузки операций, которые мы обсудим более подробно в последующих 
главах. Такие методы обычным образом наследуются в деревьях классов и содержат 
в начале и конце своих имен по два символа подчеркивания, чтобы акцентировать 
внимание на их особенности.
'''

'''
имеются два вида объектов: объекты классов и 
объекты экземпляров. Объекты классов обеспечивают стандартное поведение и служат 
фабриками для объектов экземпляров. Объекты экземпляров являются действитель­
ными объектами, обрабатываемыми вашей программой — каждый представляет собой 
самостоятельное пространство имен, но наследует имена от класса, из которого он был создан. 
Объекты классов происходят из операторов, а экземпляры — из вызовов; при каждом обращении к классу вы получае­те
